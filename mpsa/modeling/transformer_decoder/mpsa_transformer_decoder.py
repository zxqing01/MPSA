...
class LocalRepresentation(nn.Module):
    ...
    def __init__(self, d_model=256):
        ...
        self.conv_1x1 = nn.Conv2d(d_model, d_model, kernel_size=1, groups=d_model)
        self.conv_3x3 = nn.Conv2d(d_model, d_model, kernel_size=3, padding=1, groups=d_model)
        self.conv_5x5 = nn.Conv2d(d_model, d_model, kernel_size=5, padding=2, groups=d_model)
        # Final channel reduction
        self.projection = nn.Conv2d(d_model * 3, d_model, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # Reduce channels back to d_model
        x = self.projection(x)

        # Reshape the local features and permute dimensions for linear transformation
        return self.out(x.view(B, self.d_model, H*W).permute(0, 2, 1))


class MPSA_CA(nn.Module):
    """
    ...
    Methods:
        ...
        forward(self, q, x, memory_mask, pos, query_pos): Forward pass through the MPSA_CA module.
    """
    ...
    def tokens(self, x, q, mask=None):
        ...
        invalid_queries_mask = (torch.isnan(sim) | torch.isinf(sim)).all(dim=2)
        most_similar_indices = torch.argmax(sim, dim=2)
        tokens = torch.gather(x, 2, most_similar_indices.unsqueeze(-1).expand(-1, -1, -1, D))
        tokens = tokens * (1 - invalid_queries_mask.unsqueeze(-1).expand(-1, -1, -1, D) * 1)

        selected_scores = torch.gather(sim, 2, most_similar_indices.unsqueeze(-1).expand(-1, -1, -1, Q))
        selected_scores = torch.where(torch.isfinite(selected_scores), selected_scores,
                                       torch.zeros_like(selected_scores))
        diagonal_weights = torch.diagonal(selected_scores, dim1=-2, dim2=-1).unsqueeze(-1).expand(-1,-1,-1,D).permute(0, 2, 1, 3).reshape(B, Q, C)
        selected_tokens=(tokens).permute(0, 2, 1, 3).reshape(B, Q, C)
        combined_features = nn.functional.normalize(self.proj(diagonal_weights), dim=1)*self.alpha + selected_tokens
    ...
    def forward(self, tgt, memory, memory_mask, pos, query_pos):
        ...
        # Find the feature token to each query
        memory = self.tokens(memory, tgt, memory_mask)  # BxQxD

        out = self.final(memory)  # BxQxD
